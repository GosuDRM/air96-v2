...
#define RF_LONG_PRESS_DELAY   60
#define DEV_RESET_PRESS_DELAY 60
#define RGB_TEST_PRESS_DELAY  60
...
void long_press_key(void)
{
    ...
    if (f_dev_reset_press) {
        ...
        if (dev_reset_press_delay >= DEV_RESET_PRESS_DELAY)  {
            ...
            wait_ms(500);
            uart_send_cmd(CMD_CLR_DEVICE, 10, 5);
...
bool process_record_kb(uint16_t keycode, keyrecord_t *record) {
    ...
    switch (keycode) {
        ...
        case LNK_BLE2:
        case LNK_BLE3: {
            /* channel constants are sequential: LINK_RF_24=0, LINK_BT_1=1, LINK_BT_2=2, LINK_BT_3=3 */
            uint8_t ch = keycode - LNK_RF;
            if (record->event.pressed) {
                if (dev_info.link_mode != LINK_USB) {
                    rf_sw_temp    = ch;
                    f_rf_sw_press = 1;
                ...
                f_rf_sw_press = 0;
                if (rf_sw_press_delay < RF_LONG_PRESS_DELAY) {
                    dev_info.link_mode   = ch;
                    dev_info.rf_channel  = ch;
                    dev_info.ble_channel = ch;
                    uart_send_cmd(CMD_SET_LINK, 10, 5);
                }
            }
            return false;
